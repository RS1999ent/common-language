...
public class CommonLanguage implements IOFMessageListener, IFloodlightModule {
	...
	protected ArrayList<Entry<DatapathId , OFPort>> borderSwitches = new ArrayList<Entry<DatapathId, OFPort>>();

	private static final byte[] BOOTSTRAP_STANDARD_MAC = HexString
			.fromHexString("11:11:11:11:11:0e");
	...
	public void init(FloodlightModuleContext context)
		...
		staticRulePusherService = context
	...
	protected Command handlePacketIn(DatapathId sw, OFPacketIn pi,
		...
		// System.out.println("HANDLING PACKET IN");
		...
		if (eth.getPayload() instanceof BSN) {
			...
			BootstrapAdvert advert = null;

			advert = (BootstrapAdvert) eth.getPayload();
			// logger.info("Bootstrap IP received is {}",
			// advert.getNeighborIP());
			return handleBootstrapAdvert(advert, sw, inPort, cntx, pi);
		}
		// logger.info("")
		return Command.CONTINUE;
	}

	private Command handleBootstrapAdvert(BootstrapAdvert advert,
			DatapathId sw, OFPort inPort, FloodlightContext cntx, OFPacketIn pi) {
		Set<DatapathId> allSwitches = switchService.getAllSwitchDpids();
		boolean broken = false;
		boolean isBorderSwitch = false;
		
		//maybe a better way to do this
		for(Entry<DatapathId, OFPort> aEntry: borderSwitches)
		{
			if(aEntry.getKey().compareTo(sw) == 0)
			{
				isBorderSwitch = true;
				break;
			}
		...
		logger.info("PROCESSING BOOTSTRAPADVERT ON SW {}", sw);
		// logger.info("HANDLE BOOT STRAP SW {}", sw);
		for (DatapathId aSwitch : allSwitches) {
			if (aSwitch.compareTo(sw) != 0) {
				...
				if (toBorder != null) {
					...
					IPv4Address address = IPv4Address
							.of(advert.getNeighborIP());
					mb.setExact(MatchField.IPV4_DST, address);
					mb.setExact(MatchField.ETH_TYPE, EthType.IPv4);	
					pushRoute(toBorder, mb.build(), pi, cntx, true,
							OFFlowModCommand.ADD);
				} else {
					logger.info("toBorder for switch {} to {} null", aSwitch,
							sw);
					broken = true;
					break;
				}
		...
		if (!broken) {
			Match.Builder mb = OFFactories.getFactory(pi.getVersion())
					.buildMatch();
			IPv4Address address = IPv4Address.of(advert.getNeighborIP());
			mb.setExact(MatchField.IPV4_DST, address);
			mb.setExact(MatchField.ETH_TYPE, EthType.IPv4);
	...
	private Command handleLldp(LLDP lldp, DatapathId sw, OFPort inPort,
		...
		for (LLDPTLV lldptlv : lldp.getOptionalTLVList()) {
			if (lldptlv.getType() == 127 && lldptlv.getLength() == 12
				...
				remoteSwitch = switchService.getSwitch(DatapathId.of(dpidBB
						.getLong(4)));
		...
		if (fromOutsideDomain) {
			logger.info("HANDLING LLDP FROM OUTSIDE DOMAIN ON SWITCH {}", sw);
			OFPacketOut out = generateBootstrapPacket(iofSwitch, inPort);
			boolean borderSwitchInList = false;
			
			//probably a better way to do this. like isIn method fro array list. 
			//don't know how that works with entry types.  Also, datapathIDs might not
			//be directly comparable.
			for(Entry<DatapathId, OFPort> aBorderSwitch: borderSwitches)
			{
				if(aBorderSwitch.getKey().compareTo(sw) == 0)
				{
					borderSwitchInList = true;
					break;
				}
			}
			if(!borderSwitchInList)
			{
				Entry<DatapathId, OFPort> entry = new SimpleEntry<>(sw, inPort);
				borderSwitches.add(entry);
			}
			iofSwitch.write(out);
			// iofSwitch.flush();
	...
	private OFPacketOut generateBootstrapPacket(IOFSwitch iofSwitch,
			OFPort inPort) {

		DatapathId sw = iofSwitch.getId();
		// if (logger.isTraceEnabled()) {
		logger.info("Sending bootstrap packet out of switch: {}, port: {}",
		...
		ethernet = new Ethernet().setSourceMACAddress(ofpPort.getHwAddr())
				...
				.setEtherType(EthType.of(Ethernet.TYPE_BOOTSTRAP & 0xffff)); // see
																				// generatelldp
																				// in
																				// linkleveldiscoverymanager.java
		ethernet.setPayload(bootstrap);

		byte[] data = ethernet.serialize();
		OFPacketOut.Builder pob = switchService.getSwitch(sw).getOFFactory()
				.buildPacketOut();
		...
		actions.add(iofSwitch.getOFFactory().actions().buildOutput()
	 ...
	 * @return srcSwitchIncluded True if the source switch is included in this
	 *         route
	 */

	public boolean pushRoute(Route route, Match match, OFPacketIn pi,
			FloodlightContext cntx, boolean doFlush,
		...
		for (int indx = 0; indx < switchPortList.size() - 1; indx++) {
			...
			if (sw == null) {
				...
				logger.info("Unable to push route, switch at DPID {} "
			...
			OFPort inPort = OFPort.ANY;// switchPortList.get(indx -
										// 1).getPortId();
	...
	public void pushOutBorder(DatapathId dpid, OFPort inPort, Match iMatch) {
		/*
		 * Insert static flows on all ports of the switch to redirect DHCP
		 * client --> DHCP DHCPServer traffic to the controller. DHCP client's
		...
		for (OFPortDesc port : sw.getPorts()) {
			if (port != sw.getPort(inPort)) {
				...
				staticRulePusherService.addFlow(
						"border-port---" + port.getPortNo().getPortNumber()
								+ "---(" + port.getName() + ")", flow.build(),
						sw.getId());
			}
		}
	}
